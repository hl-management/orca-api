...
RSpec.describe OrcaApi::PatientService::Income, :orca_api_mock do
  ...
  def expect_orca12_patientmodv34_99(path, body, prev_response_json)
    ...
    load_orca_api_response("orca12_patientmodv34_income_99.json")
  end
  ...
  describe "#get" do
    ...
    context "正常系" do
      let(:patient_id) { 4 }
      let(:response_json) { load_orca_api_response("orca12_patientmodv34_income_01.json") }
      ...
      its(:income_information) { is_expected.to eq(response_data.first[1]["Income_Information"]) }
    end

    context "異常系" do
      let(:patient_id) { 2000 }
      let(:response_json) { load_orca_api_response("orca12_patientmodv34_income_01_E10.json") }
  ...
  describe "#update" do
    ...
    context "正常系" do
      ...
      shared_examples "結果が正しいこと" do
        ...
        describe "[#{attr_name.inspect}]" do
          ...
          it { is_expected.to eq(parse_json(updated_response_json).first[1][attr_name]) }
        end
      end

      context "更新する" do
        let(:get_response_json) { load_orca_api_response("orca12_patientmodv34_income_01_modify.json") }
        let(:updated_response_json) { load_orca_api_response("orca12_patientmodv34_income_02_modify.json") }
      ...
      context "削除する" do
        let(:get_response_json) { load_orca_api_response("orca12_patientmodv34_income_01_delete.json") }
        let(:updated_response_json) { load_orca_api_response("orca12_patientmodv34_income_02_delete.json") }
    ...
    context "異常系" do
      let(:get_response_json) { load_orca_api_response("orca12_patientmodv34_income_01_modify.json") }
      let(:updated_response_json) { load_orca_api_response("orca12_patientmodv34_income_02_modify.json") }
      let(:abort_response_json) { load_orca_api_response("orca12_patientmodv34_income_99.json") }
        ...
        before do
          ...
          expect(orca_api).to receive(:call).with("/orca12/patientmodv34", body: instance_of(Hash)).exactly(3) { |_, body:|
            ...
            when 2
              ...
              data.first[1]["Api_Result"] = "E61"
              data.first[1]["Api_Result_Message"] = "低所得者２更新エラー"
              data.to_json
            when 3
              expect(req["Request_Number"]).to eq("99")
